...
export default function ReplayDock({ currentAction, currentStep, totalSteps, stages, isPlaying, isSnapshot, speed,
  ...
  const index = totalSteps && !isSnapshot ? Math.min(currentStep + 1, totalSteps) : 0;
  const percent = totalSteps ? index / totalSteps * 100 : 0;
  const title = pendingLabel || (isSnapshot && totalSteps ? `已保存 ${totalSteps} 条记录 · 尚未开始回放` : currentAction?.title || '无动作记录');
  return (
    <section className="replay-dock" aria-label="会话回放控制">
      <div className="replay-dock-head">
        <span className="replay-label"><CaretRight size={13} weight="fill" />{isSnapshot ? '历史会话' : '回放'}</span>
        <span className="replay-current-title" title={title}>{title}</span>
      ...
      <div className="replay-dock-controls">
        ...
        <div className="replay-slider-wrap"><div className="replay-slider-track" aria-hidden="true"><span style={{ width: `${percent}%` }} /></div><input type="range" min={0} max={Math.max(0, totalSteps - 1)} value={totalSteps && !isSnapshot ? currentStep : 0} disabled={blocked} onChange={(event) => onStepChange(Number(event.target.value))} aria-label="回放进度" aria-valuetext={isSnapshot ? `尚未开始回放，共 ${totalSteps} 条已保存记录` : `第 ${index} 条，共 ${totalSteps} 条`} /></div>
        <span className="replay-time">{pendingLabel ? '时间：未读取' : isSnapshot ? '尚未回放' : currentAction?.timeOffset ?? '时间：无'}</span>
