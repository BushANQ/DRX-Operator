...
integration with all DRX-AGENT subsystems.
"""
...
class MasterAgent:
    ...
    def _build_system_prompt(self) -> str:
        ...
        return (
            "你是 DRX-AGENT，一个自主红队渗透测试专家系统。\n"
            f"你由 {model_name} 模型驱动。\n"
            "【身份】当被问及「你是谁 / 你是什么模型」时，"
            f"如实回答：你是 DRX-AGENT，底层模型是 {model_name}。\n"
    ...
    def _tool_http_fetch(
        ...
        try:
            ...
            req = urllib.request.Request(
                ...
                headers={"User-Agent": "DRX-AGENT/0.5"},
            )
    ...
    def _tool_web_search(self, query: str, max_results: int = 10) -> str:
        ...
        try:
            req = urllib.request.Request(
                api_url, headers={"User-Agent": "DRX-AGENT/0.5"}
            )
            try:
    ...
    def _tool_cve_lookup(cve_id: str) -> str:
        ...
        try:
            req = urllib.request.Request(
                url, headers={"User-Agent": "DRX-AGENT/0.5 (CVE lookup)"}
            )
            try:
    ...
    def _tool_generate_report(
        ...
        try:
            ...
            if not title:
                title = f"DRX-AGENT Pentest Report {stamp}"
            ...
            lines.append("---")
            lines.append("_Generated by DRX-AGENT._")
            md_body = "\n".join(lines)
